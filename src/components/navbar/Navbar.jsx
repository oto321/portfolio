...
function NavBar({ orientation, activeSection }) {
  function Item({ image: ImageComponent, text }) {
    const [hover, setHover] = useState(false);

    const hoverFlip = (isHovering) => {
      setHover(isHovering);
    };
      ...
      <li style={{ fontFamily: "Orbitron" }} onMouseEnter={() => hoverFlip(true)} onMouseLeave={() => hoverFlip(false)}>
        <a
          ...
          className={`
            ...
            ${text.toLowerCase() === activeSection ?
    ...
    <div className="flex ">
      ...
      `}>
        <ul className={` gap-5 flex ${orientation ?
          ...
          <Item image={ToolsIcon} text="Tools" />
